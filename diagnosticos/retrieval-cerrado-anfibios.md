...
1. **Falta de documento consolidado sobre herpetofauna do Cerrado.**
   O ICMBio organiza os PANs de Herpetofauna por *recorte biogeográfico regional*, não por bioma: existem PAN Nordeste, PAN Sul, PAN Sudeste, PAN Espinhaço, PAN Insular. Nenhum se chama "PAN Herpetofauna do Cerrado" — confirmado na seção "Cobertura do corpus" abaixo. Isso não é uma falha de indexação; é assim que a fonte primária organiza os planos. Quando alguém pergunta "quais anfíbios ocorrem no Cerrado", não existe *um documento* que sintetize essa resposta, como existiria para a região Sul. A informação está lá, mas espalhada: cada espécie tem sua própria ficha no SALVE (base de avaliação de risco de extinção), e o campo `Bioma` de cada ficha pode incluir "Cerrado" junto com outros biomas. É informação dispersa em milhares de fichas atômicas, não um documento-lista pronto.
...
3. **Confirmar cobertura do corpus e decidir a política de resposta quando não há documento-síntese.**
   Ideia: em vez de mexer no algoritmo de busca, primeiro confirmar manualmente (fora do escopo desta tarefa) se realmente não existe nenhum PAN de Herpetofauna do Cerrado disponível para coleta/indexação — olhando `01_fontes_web/pans` e o inventário (`06_inventario/inventario_fontes.xlsx`) — e, junto com isso, decidir o que o chatbot deve responder quando não há documento-síntese: agregar várias fichas SALVE numa resposta ("segundo o SALVE, as seguintes espécies têm Cerrado como um dos biomas: ...") em vez de tentar achar um único "documento perfeito".
   Prós: ataca a causa raiz (hipótese 1) — pode ser que o retrieval já esteja "certo" e nenhuma melhora de ranking resolva totalmente uma pergunta para a qual não existe PAN específico; o ajuste real seria de expectativa/política de resposta, não de algoritmo.
   Contras: pode confirmar que a resposta correta é "não há PAN dedicado, mas várias espécies têm Cerrado como bioma" — isso é uma resposta legítima do sistema, não um bug de indexação, e precisa ser tratada na lógica de geração (fora do escopo desta tarefa); não resolve o problema de ranking para outras perguntas que têm documento-fonte disponível mas continuam sendo atropeladas pelo boilerplate (hipóteses 2 e 4 continuam valendo independentemente).

4. **Roteamento/priorização de fonte para perguntas do tipo "lista de espécies por bioma".**
   Ideia: adicionar uma etapa leve (regra simples ou classificador pequeno) que detecta o padrão "pergunta pedindo lista de espécies por bioma/táxon" e, nesse caso, prioriza ou filtra automaticamente por `fontes=["salve"]` — sem tocar em embeddings nem no chunking.
   Prós: mais barato e rápido de implementar que as opções 1 e 2; o caso 4 do diagnóstico já mostra que esse filtro sozinho resolve boa parte do problema para esse tipo específico de pergunta.
   Contras: é um "remendo" para um padrão de pergunta específico — perguntas ambíguas (ex.: "o que diz o PAN sobre o Cerrado?") podem ser roteadas errado, e o problema de ranking geral (boilerplate vencendo textos específicos) continua existindo para perguntas fora desse padrão.
