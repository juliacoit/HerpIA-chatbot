...
# Cada flag é (chave, regex, severidade, descrição). A severidade de um
# documento é a mais alta entre todos os flags encontrados nele.
# Os padrões são propositalmente amplos (preferem falso positivo a falso
# negativo) — quem filtra de verdade é a revisão humana, isto é só triagem.
FLAGS = [
    ("cpf", re.compile(r"\b\d{3}\.\d{3}\.\d{3}-\d{2}\b"), "ALTA", "Possível CPF"),
    ("email", re.compile(r"\b[\w.+-]+@[\w-]+\.[\w.-]+\b"), "ALTA", "E-mail pessoal"),
    ("telefone", re.compile(r"\(\d{2}\)\s?\d{4,5}-?\d{4}\b"), "MEDIA", "Telefone"),
    (
        "coordenada_decimal",
    ...
    ("utm", re.compile(r"\bUTM\b"), "MEDIA", "Menção a sistema UTM"),
    (
        "palavra_restrito",
        re.compile(
...
def analisar_documento(doc: dict) -> list[dict]:
    ...
    for pag in paginas:
        ...
        for chave, regex, severidade, descricao in FLAGS:
